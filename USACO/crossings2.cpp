...
int main(void)
{
	ifstream infile;
	char whichFil[30];
	cin.getline(whichFil, 29);
	infile.open(whichFil);
	...
	for (int i = 0; i < N; i++)
	{
		...
		for (int k = 0; k < i; k++)
		{
			...
			if (x0[i] == x1[i])
			{
				...
				if (x1[k] != x0[k])
				{
					int val = (double) ((1-0)/(x1[k]-x0[k]))*(x0[i]-x0[k]);
					if ((val > 0) && (val < 1))
					{
						if (notCheck[k] == ';')
							notSafe++;
			...
			else if ((1/(x1[i]-x0[i])) == (1/(x1[k]-x0[k])))
				continue;
			...
			{
				double xVal = (double) ( (x0[i]/(x1[i]-x0[i])) - (x0[k]/(x1[k]-x0[k])) ) / ( (1.0 / (x1[i]-x0[i])) - (1.0 / (x1[k]-x0[k])) );
				double yVal = (double) 1 + ((1.0/(x1[i]-x0[i])) * (xVal-x1[i]));
				//cout << i << " " << k << " " << xVal << " " << yVal << "\n";
				if ((yVal > 0.0) && (yVal < 1.0))
				{
					//cout << i << " " << xVal << " " << yVal << " " << k << " " << notCheck[k] << " Choice 3\n";
					if (notCheck[k] == ';')
						notSafe++;
					else
						notSafe += 2;
